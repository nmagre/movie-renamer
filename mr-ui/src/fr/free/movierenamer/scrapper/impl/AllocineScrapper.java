...
public class AllocineScrapper extends MovieScrapper {
  ...
  protected List<Movie> searchMedia(String query, Locale locale) throws Exception {
    ...
    JSONObject json = WebRequest.getJsonDocument(searchUrl);
  ...
  protected MovieInfo fetchMediaInfo(Movie movie, Locale locale) throws Exception {
    ...
    fields.put(MovieProperty.posterPath, JSONUtils.selectString("href", JSONUtils.selectObject("poster", movieObject)));
    ...
    if (medias != null) {
      for (JSONObject media : medias) {
        if ("picture".equals(JSONUtils.selectString("class", media))) {
          ...
          if (code == 31001) {
            // affiche
            imageFields.put(ImageProperty.category, ImageCategoryProperty.thumb.name());
          } else if (code == 31006) {
            // photo
            imageFields.put(ImageProperty.category, ImageCategoryProperty.fanart.name());
          }
