...
import fr.free.movierenamer.scraper.impl.image.FanartTVshowImagesScraper;
 ...
 */
public abstract class TvShowScraper extends MediaScraper<TvShow, TvShowInfo> {

  protected TvShowScraper(AvailableLanguages... supportedLanguages) {
  ...
  protected final List<ImageInfo> fetchImagesInfo(TvShow tvshow) throws Exception {
    ...
    FanartTVshowImagesScraper fanartImagesSc = new FanartTVshowImagesScraper();
    List<ImageInfo> tmpImagesInfo = fanartImagesSc.getImages(tvshow);
    if (tmpImagesInfo != null) {
      imagesInfo.addAll(tmpImagesInfo);
    }

    // use scraper default get image
    if (imagesInfo.isEmpty()) {
      tmpImagesInfo = getScraperImages(tvshow);
      if (tmpImagesInfo != null) {
        imagesInfo.addAll(tmpImagesInfo);
      }
    }
  ...
  @Override
  public Media.MediaType getSupportedMediaType() {
